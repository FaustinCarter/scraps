...
class Resonator(object):
    ...
    def do_lmfit(self, fitFn, label="default", fit_type="IQ", **kwargs):
        ...
        self.lmfit_result[label]["values"] = np.asarray(
            [val.value for key, val in lmfit_result.params.items() if val.vary is True]
        )
        self.lmfit_result[label]["labels"] = [
            key for key, val in lmfit_result.params.items() if val.vary is True
        ]
    ...
    def do_emcee(self, fitFn, label="default", **kwargs):
        ...
        emcee_vals = np.asarray(
            [
                np.percentile(emcee_result.flatchain[key], 50)
                for key in emcee_result.flatchain.keys()
            ]
        )
        err_plus = np.asarray(
            [
                np.percentile(emcee_result.flatchain[key], 84)
                for key in emcee_result.flatchain.keys()
            ]
        )
        err_minus = np.asarray(
            [
                np.percentile(emcee_result.flatchain[key], 16)
                for key in emcee_result.flatchain.keys()
            ]
        )
        ...
        self.emcee_result[label]["emcee_sigmas"] = list(
            zip(err_plus - emcee_vals, emcee_vals - err_minus)
        )

        # It is also useful to have easy access to the maximum-liklihood estimates
        self.emcee_result[label]["mle_vals"] = emcee_result.flatchain.iloc[
            np.argmax(emcee_result.lnprob)
        ]
    ...
    def burn_flatchain(self, num_samples=0, label="default"):
        ...
        for ix, chain in enumerate(chains.T):
            flatchain_with_burn[self.emcee_result[label]["mle_labels"][ix]] = chain[
                num_samples:
            ].flat
        ...
        # Make a list of tuples that are (+err, -err) for each paramter
        self.emcee_result[label]["emcee_sigmas_burn"] = list(
            zip(err_plus - emcee_vals, emcee_vals - err_minus)
        )
...
def makeResFromData(dataDict, paramsFn=None, fitFn=None, fitFn_kwargs=None, paramsFn_kwargs=None):
    ...
    assert all(
        key in dataDict.keys() for key in expectedKeys
    ), "Your dataDict is missing one or more keys"
